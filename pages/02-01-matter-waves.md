...
먼저 한 가지 확인하고 넘어가봅시다. 1.2절에서 빛이 $E=h\nu$의 에너지 덩어리를 가지는 입자라고 했었죠. 그러면 운동량을 가지고 있을 겁니다. 우리가 보통 입자라고 생각하는 것들은 $p=mv$라는 운동량을 가지고 있죠. 근데 빛의 질량은 얼마일까요? Einstein의 특수 상대성 이론에 따르면 빛의 정지질량은 0입니다. 아쉽게도 빛의 운동량은 이 식으로 계산할 수는 없을 것 같습니다...  
하지만 특수 상대성 이론의 또 다른 식에 따르면 정지질량이 0인 입자의 에너지와 운동량은 $E=pc$의 관계에 있다고 합니다. 여기에 빛의 에너지를 넣어보죠.

$$
p = \frac{h\nu}{c} = \frac{h}{\lambda}
$$

계산은 간단하지만 정말로 그럴까요? 빛이 정말로 이만큼의 운동량을 가지고 있는 지는 실험으로도 확인할 수 있어야 합니다.  

1923년, 미국의 물리학자 Compton이 이를 밝혀냅니다. Compton은 X선을 흑연에 쏘고 산란된 X선을 관찰하였습니다. 그런데 이상한 일이 벌어졌습니다. 산란되어 나온 X선의 파장이 원래보다 길어져 있었던 것이죠. 고전물리학으로는 설명이 안 됩니다. 빛이 산란된다고 파장이 변하지는 않거든요. 그런데 빛을 입자, 즉 광자로 보면 이야기가 달라집니다. 광자는 운동량을 가지고 있기 때문에 전자와 부딪히면 원래 가지고 있던 에너지의 일부를 전자에게 넘겨주면서 튕겨 나올 때는 에너지를 잃고 파장이 길어지는 겁니다. 에너지와 운동량의 보존을 적용하면 파장의 변화는 이렇게 계산할 수 있습니다.

$$
\Delta\lambda = \frac{h}{m_ec}(1-\cos\theta)
$$

여기서 $\theta$는 산란각이고, $\frac{h}{m_ec}$를 Compton 파장이라고 부릅니다. 한번 계산해보죠.
...
lambda_C = h / (m_e * c)
print(f"Compton 파장 = {lambda_C * 1e12:.4f} pm")

lam0 = 71.1e-12  # Compton이 사용한 Mo K-alpha X선의 파장
print(f"원래 X선 파장: {lam0 * 1e12:.1f} pm")

for deg in [45, 90, 135]:
    dlam = lambda_C * (1 - np.cos(np.deg2rad(deg)))
    print(
        f"{deg:3d}도 산란 -> 파장 {(lam0 + dlam) * 1e12:.2f} pm ({dlam / lam0 * 100:.1f}% 증가)"
    )
```
```
Compton 파장 = 2.4263 pm
원래 X선 파장: 71.1 pm
 45도 산란 -> 파장 71.81 pm (1.0% 증가)
 90도 산란 -> 파장 73.53 pm (3.4% 증가)
135도 산란 -> 파장 75.24 pm (5.8% 증가)
```
Compton의 측정값은 이 예측과 잘 맞아떨어졌습니다. 그리고 Compton은 이 공로로 1927년에 노벨 물리학상을 받습니다.

실제로 X선의 운동량을 계산해보면 $p = h / \lambda = 9.32 \times 10^{-24}$ kg m/s입니다. X선의 에너지는 전자기파 중에서 큰 편에 속하지만 이 정도 에너지를 가진 광자를 맞는다고 아프지는 않겠네요...


## de Broglie의 물질파

1924년, 프랑스의 de Broglie는 박사 학위 논문을 제출합니다. 분량이 얇았고, 심사위원들은 이걸 어떻게 판단해야 할지 몰라 곤란해했다고 합니다. 결국 Einstein에게 의견을 물었고, Einstein은 "그는 거대한 장막의 한 귀퉁이를 들어 올렸다"고 답했습니다. de Broglie가 한 일은 질문을 거꾸로 생각해본 것입니다. 빛을 생각해봅시다. 200년 넘게 파동이라고 믿어왔는데, 광전 효과에서는 입자처럼 행동했습니다. 그러니까 빛은 파동인 줄 알았는데 입자성도 가지고 있었던 겁니다. 그러면 반대로, 입자인 줄 알았던 전자도 사실은 파동성을 가지고 있는 것 아닐까요? 이상하게 들릴 수도 있겠지만 완전히 터무니없는 생각 같지는 않습니다.

전자가 파동이라면 파장이 있어야겠죠. 그런데 대체 얼마일까요? de Broglie는 광자의 관계식에서 힌트를 얻습니다. Compton 산란으로 확인된 식을 변형하면 $\lambda = \frac{h}{p}$가 되고, 이는 곧 운동량은 파장과 대응된다는 뜻입니다. de Broglie는 이렇게 생각했습니다. 이 관계가 빛에만 적용될 이유가 있을까? 만약 입자도 파동성을 나타낸다면 이 식을 따르지 않을까?  
속도 $v$로 운동하는 질량이 $m$인 입자의 운동량이 $p=mv$로 주어진다는 것은 잘 알고 계실 겁니다.  
...
정말 de Broglie 공식과 맞는지 확인해봅시다. 파장을 두 가지 방법으로 구해서 비교하면 됩니다. 회절 무늬로부터 파장을 구하는 방법은 표면 회절 격자 공식($n\lambda = D \sin \phi$)을 이용하면 됩니다.
...
그런데 이 아이디어에는 실험 확인보다 더 놀라운 결과가 딸려 있었습니다. 1장에서 Bohr가 설명하지 못하고 남겨둔 그 가정, 각운동량이 왜 $\hbar$의 정수배인가에 대한 답이 여기서 저절로 나옵니다. 다음 절에서 이야기를 계속해보죠.
