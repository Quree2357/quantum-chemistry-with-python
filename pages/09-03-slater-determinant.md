...
$$
\Psi = |\chi_1,\chi_2,\cdots,\chi_N \rangle = |1,2,\cdots,N \rangle
$$

Dirac의 브라켓 표기법에서 켓 기호만 따와서 썼죠. $\chi$를 쓰는 것도 귀찮아서 그냥 숫자만 쓰기도 합니다. 이 표기법은 17장 이후에 많이 보시게 될 겁니다. 예를 들어, 헬륨 원자의 바닥 상태는 $|1s\alpha, 1s\beta \rangle$ 또는 $|1s, \bar{1s} \rangle$라고 쓰게 됩니다.
[[/TIP]]
...
실제로 얼마나 부족하냐면 헬륨 원자의 바닥 상태 에너지를 오비탈 근사의 테두리에서 최대한 잘 계산해도 -77.49 eV밖에 안 나오는데 실험값은 -79.00 eV거든요. 9.1절에서 전자 간 상호작용을 통째로 무시했을 때보다는 훨씬 작은 오차지만 2 eV 정도 차이면 화학 결합 한 개분을 잘못 예측하거나 반응 엔탈피가 뒤바뀌는 정도라서 그냥 넘길 수는 없습니다. 이 오차를 **전자 상관 에너지(electron correlation energy)**라고 부르는데 이 책의 후반부의 상당 부분이 어떻게 이 차이를 줄여나갈 수 있을까에 대한 얘기입니다.
