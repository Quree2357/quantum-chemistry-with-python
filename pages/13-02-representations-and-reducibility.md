...
[[-0.5   -0.866]
 [-0.866  0.5  ]]
```
두 결과가 다릅니다. 13.1절에서 얘기했던 '돌리고 뒤집은 것과 뒤집고 돌린 것이 다르다'는 게 바로 이거죠. 이렇게 교환되지 않는 두 행렬은 같은 기저로 동시에 대각화할 수 없습니다. 그래서 더 쪼개지지 않는 거죠.  

이렇게 더 쪼갤 수 없는 경우를 **기약 표현(irreducible representation)**이라고 합니다. 마치 자연수를 소인수분해하는 것과 비슷하죠. 기약 표현은 점군의 고유한 성질입니다. 같은 점군은 같은 기약 표현을 갖죠. 그래서 이 기약 표현들에는 **대칭 종(symmetry species)**이라는 이름을 붙입니다. 방금 우리가 떼어낸 조각들이 각각 어떤 대칭 종인지 말할 수 있다는 뜻이죠. 오비탈이든 진동 모드든 이렇게 분류할 수 있습니다.
