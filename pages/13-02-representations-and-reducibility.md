...
암모니아 분자의 표현 행렬은 기저를 잘 잡아서 (1, 1, 2)차원 조각들로 쪼갤 수 있었습니다. 만약 다른 기저를 택한다면 더 작은 조각으로 쪼갤 수 있을까요? 답은 '아니다'입니다. 마지막에 나타난 2차원 행렬은 어떤 기저를 잡아도 더 이상 쪼갤 수 없거든요. $C_3$ 조작과 $\sigma_a$ 조작을 다른 순서로 해보겠습니다.
```python
C3 = (T.T @ orb_reps["C3"] @ T)[2:, 2:]
sigma_a = (T.T @ orb_reps["σ_a"] @ T)[2:, 2:]
...
[[-0.5       -0.8660254]
 [-0.8660254  0.5      ]]
```
두 결과가 다릅니다. 13.1절에서 얘기했던 '돌리고 뒤집은 것과 뒤집고 돌린 것이 다르다'는 게 바로 이거죠. 이렇게 교환되지 않는 두 행렬은 같은 기저로 동시에 블록 대각화할 수 없습니다. 그래서 더 쪼개지지 않는 거죠.  

이렇게 더 쪼갤 수 없는 경우를 **기약 표현(irreducible representation)**이라고 합니다. 마치 자연수를 소인수분해하는 것과 비슷하죠. 기약 표현은 점군의 고유한 성질입니다. 같은 점군은 같은 기약 표현을 갖죠. 그래서 이 기약 표현을 이용해 점군들을 또 분류할 수 있고, 이렇게 분류된 것을 **대칭 종(symmetry species)**이라고 합니다. 다음 절에서 대칭 종의 분류에 대해 살펴보고 이것이 왜 중요한지 알아보겠습니다.
