...
4장에서 마디가 있으면 에너지가 높아진다고 했던 것 기억나시나요? 비슷하게 마디면이 있는 반결합성 오비탈의 에너지가 더 높습니다. 그리고 일반화학의 설명대로 결합성 오비탈에서는 원자핵 사이에서 전자 밀도가 높게 나타나고 있네요. 이름의 유래가 바로 여기서 온 것입니다.
...
자세한 중간 과정은 생략하겠지만 에너지가 최소값이 되도록 $\zeta$와 $R$의 값을 최적화하면 $\zeta$는 약 1.239, $R$은 약 2.00 bohr가 나옵니다. 결합 길이가 실험값과 거의 일치하고 있네요. 에너지 그래프를 다시 그려보죠.
```python
...
이제 수소 분자 이온에 대한 이야기는 끝입니다. 그럼 다음은 뭘까요? 당연히 전자가 두 개인 수소 분자로 가야죠. 이번에는 핵의 위치를 고정시킨다고 해도 삼체 문제가 되니 정확한 해석해를 구할 수 있는 방법이 없습니다. 그래도 어떻게든 최대한 정확하게 분자를 기술하는 방법을 살펴보죠. 모험은 계속되어야 하니까요.


## 확인 문제
1. 결합 길이 $R$을 4 bohr, 6 bohr 등으로 바꾸면서 전자 밀도 비교 그림을 그려보세요. 결합이 약해지면 어떻게 달라지나요?
2. 결합성 오비탈의 전자 밀도가 핵 위치에서 줄어드는 것을 규격화 상수로 설명해보세요.
3. 4.2절에서 시아닌 염료의 사슬이 길어질수록 흡수 파장이 길어진다고 했습니다. 이 절의 비편재화와 어떻게 연결되는지 설명해보세요.
