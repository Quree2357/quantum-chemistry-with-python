...
읽는 법은 간단합니다. 우변의 첫 항이 운동에너지, 둘째 항이 퍼텐셜 에너지, 그리고 좌변이 전체 에너지입니다. 결국 에너지 보존 법칙을 파동의 언어로 쓴 것입니다.
...
- 셋째, 그래서 해가 반드시 복소수입니다.  
고전 파동에서는 복소수가 계산 요령이었습니다. 0.1.1에서 "실수부만 꺼내면 원래 코사인으로 돌아온다"고 했었죠. 그런데 여기서는 실수부만 꺼내면 방정식을 만족하지 않습니다. 복소수가 본질입니다.
...
4. 만약 Schrödinger 방정식에서 $i$를 빼면 어떤 방정식이 될까요? (힌트: 열이 퍼지는 현상을 다루는 방정식과 같은 모양이 됩니다.)