...
앞 절에서 미분은 $h$를 너무 줄이면 반올림 오차가 증폭되어 오히려 정확도가 나빠졌습니다. 과연 적분도 그럴까요?
...
정확도가 계속 좋아집니다. 왜 그럴까요? 미분의 경우 거의 같은 두 수를 빼면서 앞자리가 상쇄되고, 이를 아주 작은 $h$로 나누면서 오차가 증폭되었습니다. 반면 수치적분은 작은 조각들의 넓이를 계속 '더해나가는' 과정이기 때문에, 작은 값으로 나누는 위험한 연산이 없습니다.

그래서 수치계산 분야에서는 이런 말이 있습니다. *적분은 쉽고 미분은 어렵다.* 수학 수업에서 배운 것과 정반대라 재미있죠. 손으로 할 때는 미분이 기계적이고 적분이 까다로웠는데, 컴퓨터에서는 뒤집힙니다.
...
이제 미분과 적분을 모두 컴퓨터로 계산할 수 있게 되었습니다. 미분이 중요한 이유는 최적화 문제를 풀 때 항상 등장하는 개념이기 때문입니다. 적분도 앞으로 이 책에서 지겹도록 나올 것이고 나중에는 이런 적분을 수십만 번 계산하게 됩니다. 다음 절에서는 이 절의 진짜 목적지로 갑니다. 미분을 행렬로 만들 차례입니다.
