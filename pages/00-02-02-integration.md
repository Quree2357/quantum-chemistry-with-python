...
이미 적분과 리만 합 이야기를 했으니 사실 절반은 아시는 셈입니다. 다만 "잘게 나눠서 더한다"는 아이디어에도 여러 가지 방법이 있습니다.
...
0.1.5에서 `np.dot(f, g) * dx`로 계산했던 게 바로 이것입니다. 리만 합의 정의 그대로죠.
...
오차가 13%나 됩니다. 별로 정확하지는 않네요... 왜 그런지는 그림으로 보면 바로 알 수 있습니다.
...
똑같이 11개의 점을 썼는데 오차가 거의 30배가 줄었습니다. 그림으로 다시 봅시다.
...
아까 있던 삼각형 빈틈이 거의 사라졌습니다. 곡선이 위로 볼록해서 여전히 아주 살짝 어긋나긴 하지만, 직사각형과는 비교가 안 되죠.


## 포물선으로 근사하기

여기서 한 발 더 나가봅시다. 사다리꼴은 두 점을 직선으로 이었습니다. 직선 대신 곡선으로 이으면 더 좋지 않을까요? **심프슨 공식(Simpson's rule)**은 세 점을 포물선으로 이어서 넓이를 구합니다. SciPy에서 사용해볼 수 있습니다.

$$
\int_a^b f(x)\,dx \approx \sum_i \frac{h}{3}\left[f(x_i) + 4f\left(\frac{x_i + x_{i+1}}{2}\right) + f(x_{i+1})\right]
$$
...
또 한 번 크게 좋아졌습니다. 사다리꼴보다 약 800배쯤 더 정확하네요.
...
이제 셋을 한눈에 비교해봅시다. 위에서 그린 그림들을 나란히 놓고, 각 방법의 오차도 함께 표시하겠습니다. 여기서도 점은 5개만 씁니다.
...
계단처럼 뭉텅뭉텅 잘라내는 직사각형, 비스듬히 따라가는 사다리꼴, 곡선에 거의 달라붙는 심프슨. 같은 점 5개를 쓰는데도 오차는 $10^{-1}$, $10^{-2}$, $10^{-4}$로 자릿수가 통째로 달라집니다.
 
(앞에서 계산한 수치와 오차가 다른 것은 여기서는 그림이 잘 보이도록 점을 5개만 썼기 때문입니다. 앞에서는 11개를 썼었죠.)
...
심프슨은 $N=10001$에서 오차가 아예 0으로 찍혔습니다. 물론 진짜 0이라서 그런 건 아니고, 컴퓨터가 표현할 수 있는 한계보다 오차가 작아진 것입니다.
...
정확도가 계속 좋아집니다. 왜 그럴까요? 앞 절에서 미분이 나빠졌던 이유는 거의 같은 두 수를 빼면서 앞자리가 상쇄되어 사라졌기 때문이었습니다. 그런데 적분은 값을 더하기만 합니다. 뺄셈이 없으니 자리 상쇄도 일어나지 않죠.
...
2. 심프슨 공식은 왜 짝수 개의 구간(홀수 개의 점)을 요구할까요? 세 점씩 묶어 포물선을 그린다는 사실로부터 생각해보세요.
