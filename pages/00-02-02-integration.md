...
rect = np.sum(y[:-1]) * h
print(f"직사각형 : {rect:.10f}   오차 = {abs(rect - exact)*100:.2f} %")
```
```
...
print(f"사다리꼴 : {trap:.10f}   오차 = {abs(trap - exact)*100:.2f} %")
```
```
...
simp = simpson(y, x=x)
print(f"심프슨   : {simp:.10f}   오차 = {abs(simp - exact)*100:.3f} %")
```
```
심프슨   : 1.0001690471   오차 = 0.017 %
```
