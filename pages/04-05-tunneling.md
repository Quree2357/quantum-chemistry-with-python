...
앞 절의 마지막 부분에서 이상한 현상을 봤습니다. 전자의 에너지가 언덕의 높이보다 낮은데도 파동 함수가 언덕 안에서 0이 아니었죠. 고전물리학에서는 있을 수 없는 일입니다. 공을 언덕 위로 굴리려면 적어도 언덕의 높이만큼의 에너지는 있어야 하고, 그것보다 에너지가 모자라면 중간에 멈췄다가 다시 내려옵니다. 언덕 안쪽에 공이 들어가 있는 상황 자체가 성립하지 않죠.
...
- **첫째, 파동 함수가 벽 밖에서 0이 아닙니다.**  
무한 우물에서는 벽에서 정확히 0이었는데, 유한 우물에서는 벽 안쪽으로 스며들어가면서 지수적으로 줄어듭니다. 이런 현상을 **침투(penetration)**라고 부릅니다. 전자가 벽 안에 있을 확률을 실제로 계산해봅시다.
...
그런데 두 준위가 갈라진다는 것은 무엇을 의미할까요? 전자를 왼쪽 우물에만 넣어놓고 시작한다고 합시다. 그 상태는 대칭 상태와 반대칭 상태를 반반 섞은 것입니다. 그런데 두 상태의 에너지가 다르니 시간이 지나면서 위상이 어긋나고, 결국 전자가 오른쪽 우물로 옮겨갑니다. 그리고 다시 왼쪽으로 돌아오죠. 이렇게 전자가 자신의 에너지보다 높은 장벽의 반대쪽으로 건너가는 현상을 **양자 터널링(quantum tunnelling)**이라고 합니다. 이 왕복 주기는 갈라짐 정도 $\Delta E$로 정해집니다.
...
```
벽이 0.2 nm면 0.18 피코초 만에 왕복하는데, 1.2 nm면 약 3.6 나노초가 걸립니다. 벽이 여섯 배 두꺼워졌을 뿐인데 시간은 20000배 늘었죠. 이것이 터널링이 거리에 극도로 민감한 이유입니다.

[[TIP]]
위 그림을 보시면 반대칭 상태의 파동 함수가 그림마다 왼쪽에서 위로 솟기도 하고 오른쪽에서 위로 솟기도 하는데요, 잘못 계산된 것이 아닙니다. 파동 함수의 부호 자체에는 물리적인 의미가 없기 때문입니다. $\psi$가 해라면 $-\psi$도 해이고 우리가 실제로 관측하는 확률은 제곱이니까 둘 다 같죠. `eigh` 함수는 계산된 파동 함수를 어느 부호로 돌려줄지 보장하지 않습니다. 왼쪽에서 솟을 수도, 오른쪽에서 솟을 수도 있는 거죠. 하지만 당연히 둘 다 올바른 답입니다. 계산할 때마다 이렇게 뒤집힐 수는 있는데 당황하지 않으셔도 됩니다.  
[[/TIP]]
...
3. 표에서 장벽이 0.2 nm 두꺼워질 때 갈라짐이 몇 배 줄어드는지 확인해보세요. STM에서 흐르는 전류는 파동 함수의 제곱에 비례합니다. 이를 고려하여 STM에서 탐침 높이를 0.1 nm 바꿀 때 전류가 몇 배 변할지 추정해보세요.
