...
- **첫째, 파동함수가 벽을 넘어갈 수 있습니다.**  
  무한 우물에서는 벽에서 정확히 0이었는데, 유한 우물에서는 벽 안쪽으로 넘어가면서 지수적으로 줄어듭니다. 이런 현상을 **양자 터널링(quantum tunnelling)**이라고 부릅니다. 전자가 벽 안에 있을 확률을 실제로 계산해봅시다.
...
벽이 두꺼워지면 갈라짐이 급격히 줄어듭니다. 0.2 nm에서 17 meV였던 것이 0.6 nm에서 0.2 meV로, 1.2 nm에서는 사실상 0이 됩니다. 벽이 조금만 두꺼워져도 급격히 사라진다는 뜻이죠.
...
벽이 0.2 nm면 0.24 피코초 만에 건너가는데, 1.2 nm면 16 나노초가 걸립니다. 벽이 여섯 배 두꺼워졌을 뿐인데 시간은 7만 배 늘었죠. 이것이 터널링이 거리에 극도로 민감한 이유입니다. 그리고 그 민감함이 실용적으로 쓰입니다.


## 실제로 일어나는 일들

- 암모니아 반전  
NH₃는 삼각뿔 모양인데, 질소가 수소 삼각형의 위에 있을 수도 아래에 있을 수도 있습니다. 두 배치 사이에 에너지 장벽이 있어 고전적으로는 넘나들 수 없죠. 그런데 관찰해보면 두 배치간에 반전이 빠르게 일어납니다. 실제로 측정된 반전 진동수는 23.786 GHz입니다.
...
- 주사터널링현미경(STM)  
뾰족한 금속 탐침을 시료 표면에 아주 가까이 가져가면 전자가 그 사이를 터널링으로 넘어가면서 전류가 흐릅니다. 그리고 방금 본 것처럼 터널링은 거리에 지수적으로 민감합니다. 탐침을 0.1 nm만 움직여도 전류가 열 배씩 바뀌죠. 그 민감함 덕분에 원자를 하나하나 볼 수 있습니다.
...
3. 터널링 왕복 주기가 벽 두께에 지수적으로 의존한다는 것을 이용하면, STM에서 탐침 높이를 0.1 nm 바꿀 때 전류가 몇 배 변할지 어림할 수 있습니다. 위 데이터로 추정해보세요.
